...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
    ...
    struct BACKUP_DX11_STATE
    {
        ...
        ID3D11ClassInstance *     PSInstances[256], *VSInstances[256], *GSInstances[
                                256];   // 256 is max according to PSSetShader documentation
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // Bind texture, Draw
                ID3D11ShaderResourceView* texture_srv = static_cast<ID3D11ShaderResourceView*>(pcmd->GetTexID());
...
static void ImGui_ImplDX11_CreateFontsTexture()
{
    ...
    io.Fonts->SetTexID(bd->pFontTextureView);
...
bool ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main", "vs_4_0", 0, 0, &
                       vertexShaderBlob, NULL)))
            return
                false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
        ...
        {
            {
              ...
              , 0
              , static_cast<UINT>(IM_OFFSETOF(ImDrawVert, pos))
          ...
          , {
              ...
              , DXGI_FORMAT_R32G32_FLOAT
              , 0
              , static_cast<UINT>(IM_OFFSETOF(ImDrawVert, uv))
              , D3D11_INPUT_PER_VERTEX_DATA
              , 0
            }
          , {
              ...
              , 0
              , static_cast<UINT>(IM_OFFSETOF(ImDrawVert, col))
              , D3D11_INPUT_PER_VERTEX_DATA
              , 0
            }
           ,
    ...
    // Create the pixel shader
    {
        ...
        if (FAILED(D3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main", "ps_4_0", 0, 0, &
...
static void ImGui_ImplDX11_DestroyWindow(ImGuiViewport* viewport)
{
    ...
    if (ImGui_ImplDX11_ViewportData* vd = static_cast<ImGui_ImplDX11_ViewportData*>(viewport->RendererUserData))
    {
        if (vd->SwapChain)
...
{
    ImGui_ImplDX11_Data*         bd = ImGui_ImplDX11_GetBackendData();
    ImGui_ImplDX11_ViewportData* vd = static_cast<ImGui_ImplDX11_ViewportData*>(viewport->RendererUserData);
    if (vd->RTView)
    {
        vd->RTView->Release();
        vd->RTView = nullptr;
