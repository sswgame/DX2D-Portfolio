...
VTX_OUT VS_PostProcess(VTX_IN _in)
{
    ...
    //output.vPosition = mul(float4(_in.pos, 1.f), g_matWVP);
    output.vUV = _in.vUV;
...
float4 PS_PostProcess(VTX_OUT _in) : SV_Target
{
    ...
    float2 vScreenUV = _in.vPosition.xy / resolution;    
    
    if (IsBind && TIME > 0.f)
    {        
        //vScreenUV.y += sin(vScreenUV.x * 3.141592f * 20.f + (accTime * 4.f)) * 0.01;
                
        _in.vUV.x += accTime * 0.1f;        
