...
void CCollisionMgr::CollisionOff(int _iLayerLeftIdx, int _iLayerRightIdx)
{
	...
	m_arrCheck[iRow][iCol] = false;
}

void CCollisionMgr::DeRegisterObject(CGameObject* pGameObject)
{
	int layerIndex = pGameObject->GetLayerIndex();
	if (nullptr == pGameObject->Collider2D()) { return; }

	for (int i = 0; i < MAX_LAYER; ++i)
	{
		if (m_arrCheck[layerIndex][i] || m_arrCheck[i][layerIndex])
		{
			CLayer* pLayer            = CSceneMgr::GetInst()->GetCurScene()->GetLayer(i);
			auto&   vecAllGameObjects = pLayer->GetObjects();
			for (auto& pObjects : vecAllGameObjects)
			{
				if (pObjects->Collider2D())
				{
					COLLIDER_ID id{};
					if (i < layerIndex)
					{
						id.iLeftID  = pObjects->GetID();
						id.iRightID = pGameObject->GetID();
					}
					else
					{
						id.iLeftID  = pGameObject->GetID();
						id.iRightID = pObjects->GetID();
					}
					...
					{
						iter->second = false;
						pGameObject->Collider2D()->OnCollisionExit(pObjects->Collider2D());
						pObjects->Collider2D()->OnCollisionExit(pGameObject->Collider2D());
					}
				}
			}
		}
