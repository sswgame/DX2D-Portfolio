...
int CStructuredBuffer::Create(UINT    _iElementSize
                            , UINT    _iElementCount
                            , SB_TYPE _eType
                            , bool    _bCpuAccess
                            , void*   _pInitialData)
{
	...
	if (m_bCpuAccess)
	{
		...
		m_desc_write.Usage          = D3D11_USAGE_DYNAMIC;
		m_desc_write.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
		...
		m_desc_read.Usage          = D3D11_USAGE_STAGING;
		m_desc_read.CPUAccessFlags = D3D11_CPU_ACCESS_READ;
...
void CStructuredBuffer::SetData(void* _pSrc, UINT _iElementCount)
{
	...
	memcpy(tSub.pData, _pSrc, static_cast<size_t>(m_iElementSize) * static_cast<size_t>(_iElementCount));
...
void CStructuredBuffer::GetData(void* _pDst)
{
	...
	memcpy(_pDst, tSub.pData, static_cast<size_t>(m_iElementSize) * static_cast<size_t>(m_iElementCount));
