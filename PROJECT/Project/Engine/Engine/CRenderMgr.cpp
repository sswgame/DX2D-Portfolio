...
{
	if (nullptr == m_pEditorCam) { return; }
...
void CRenderMgr::RegisterCamera(CCamera* _pCam)
{
	...
	if (-1 == _pCam->m_iCamIdx)
	{
		...
		int iIdx         = static_cast<int>(m_vecCam.size()) - 1;
	...
	else
	{
		...
		{
			m_vecCam.resize(static_cast<size_t>(_pCam->m_iCamIdx) + 1);
		}
...
void CRenderMgr::SwapCameraIndex(CCamera* _pCam, int _iChangeIdx)
{
	for (size_t i = 0; i < m_vecCam.size(); ++i)
	{
		if (_pCam == m_vecCam[i])
		{
			...
			{
				m_vecCam[_iChangeIdx]->m_iCamIdx = static_cast<int>(i);
...
void CRenderMgr::UpdateLight2D()
{
	...
	{
		m_pLight2DBuffer->Create(sizeof(tLightInfo)
