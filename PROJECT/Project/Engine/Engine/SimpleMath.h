...
namespace DirectX
{
    namespace SimpleMath
    {
        ...
        struct Vector2 : public XMFLOAT2
        {
            ...
            XM_CONSTEXPR          Vector2(int _x, int _y) : XMFLOAT2(static_cast<float>(_x), static_cast<float>(_y)) {}
            XM_CONSTEXPR          Vector2(UINT _x, UINT _y) : XMFLOAT2(static_cast<float>(_x), static_cast<float>(_y)) {}
            ...
            using f2 = float(&)[2];
            explicit operator f2() const { return (f2)x; }
        ...
        struct Color : public XMFLOAT4
        {
            ...
            explicit Color(const XMCOLOR &Packed);
                // BGRA Direct3D 9 D3DCOLOR packed color

            explicit Color(const XMUBYTEN4 &Packed);
            ...
            Color&operator= (const XMCOLOR &Packed);
            Color&operator= (const XMUBYTEN4 &Packed);
        ...
        public:
            ...
            explicit Viewport(const RECT& rct) :
                x(static_cast<float>(rct.left)), y(static_cast<float>(rct.top)),
                width(static_cast<float>(rct.right - rct.left)),
                height(static_cast<float>(rct.bottom - rct.top)),
...
namespace std
{
    ...
    template<> struct less<SimpleMath::Rectangle>
    {
        bool operator()(const SimpleMath::Rectangle&r1, const SimpleMath::Rectangle&r2) const
        {
            return ((r1.x < r2.x)
                    || ((r1.x == r2.x) && (r1.y < r2.y))
    ...
    template<> struct less<SimpleMath::Viewport>
    {
        bool operator()(const SimpleMath::Viewport&vp1, const SimpleMath::Viewport&vp2) const
        {
