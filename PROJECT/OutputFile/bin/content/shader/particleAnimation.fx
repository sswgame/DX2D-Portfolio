#ifndef _PARTICLE_ANIMATION
#define _PARTICLE_ANIMATION
...
#define BackGroundSize		g_vec2_3
...
StructuredBuffer<tAnimationParticle> g_ParticleAnim : register(t12);
...
void CS_ParticleDoor(int3 id : SV_DispatchThreadID)
{
    ...
    else
    {
        ...
        if (id.x % 2 == 0)
        {
            ParticleBuffer[id.x].scale = StartScale + (EndScale - StartScale) * lifeRatio;
        }
        else
        {
            ParticleBuffer[id.x].scale = EndScale - (EndScale - StartScale) * lifeRatio;
